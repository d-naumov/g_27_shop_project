...
public class CommonCustomerRepository implements CustomerRepository {
  ...
  public Customer save(Customer customer) {
    try (Connection connection = getConnection()) {
      ...
      String query = String.format(
          "INSERT INTO `customer` (`name`, `is_active`) VALUES ('%s', '1');"
          , customer.getName());
  ...
  public List<Customer> getAll() {
    try (Connection connection = getConnection()) {
      ...
      String query =
          "select cu.id, cu.name, ca.id, cp.product_id, p.name, p.price, p.is_active from customer as cu "
              + "left join cart as ca on cu.id = ca.customer_id "
              + "left join cart_product as cp on ca.id = cp.cart_id "
              + "left join product as p on cp.product_id = p.id where cu.is_active = 1;";
